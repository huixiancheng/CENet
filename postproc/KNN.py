...
def NN_filter(proj_range, semantic_pred, k_size=5):
    ...
    proj_unfold_range = Func.unfold(proj_range[None, None, ...], kernel_size=(k_size, k_size), padding=(pad, pad))
    proj_unfold_range = proj_unfold_range.reshape(-1, k_size * k_size, H, W)

    proj_unfold_pre = Func.unfold(semantic_pred[None, None, ...], kernel_size=(k_size, k_size), padding=(pad, pad))
