...
#         self.loss_w = np.power(self.loss_w, power_value) * np.power(10, 1 - power_value)
        ...
        # self.optimizer = optim.AdamW(self.model.parameters(), lr=0.0001, weight_decay=0.005)
    ...
    def train_epoch(self, train_loader, model, criterion, optimizer, epoch, evaluator, scheduler, color_fn, report=10,
        ...
        for i, (in_vol, proj_labels, _, _, path_seq, path_name, _, _, _, _, _) in tqdm(enumerate(train_loader), total=len(train_loader)):
...
#             optimizer.step()
            ...
            if show_scans:
                if i % self.ARCH["train"]["save_batch"] == 0:
                    ...
                    out = Trainer.make_log_img(depth_np, pred_np, gt_np, color_fn)
    ...
    def validate(self, val_loader, model, criterion, evaluator, class_func, color_fn, save_scans):
        ...
        with torch.no_grad():
            ...
            for i, (in_vol, proj_labels, _, _, path_seq, path_name, _, _, _, _, _) in tqdm(enumerate(val_loader), total=len(val_loader)):
                if not self.multi_gpu and self.gpu:
                    in_vol = in_vol.cuda()
                if self.gpu:
